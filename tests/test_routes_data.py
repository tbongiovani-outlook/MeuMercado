...
def test_callback_sucesso(auth_client, monkeypatch):
    ...
    r = auth_client.get(
        "/callback?code=abc&state=FIXEDSTATE", follow_redirects=False
    )
...
def _order(oid="OID1"):
    return {
        ...
        "order_items": [
            {"item": {"title": "Produto vendido"}, "quantity": 2, "sale_fee": 20.0}
        ],
        "shipping": {"id": 555},
    }
...
def test_mensagens_com_conteudo(auth_client, monkeypatch):
    monkeypatch.setattr(
        meli, "get_order",
        lambda oid: {"id": oid, "pack_id": "PACK1", "buyer": {"id": 9},
                     "order_items": []},
    )
    monkeypatch.setattr(
        meli, "get_pack_messages",
        lambda *a, **k: {"messages": [
            {"text": "Oi", "from": {"user_id": 9},
